...
Files placed in `~/.ssh/config.d/` will be automatically included in the main
`~/.ssh/config` via the `Include config.d/*` directive at the top of that file.

The `config.d/` directory is listed in `.gitignore` so that files there are not
tracked by default. Use `git add --force` to add a file if you want to track it.
