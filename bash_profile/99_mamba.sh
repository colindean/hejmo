...
# Cache for 1 hour (3600 seconds) since mamba configuration rarely changes
if __mamba_setup="$(bkt --ttl=3600 -- "$MAMBA_EXE" shell hook --shell bash --prefix "$MAMBA_ROOT_PREFIX" 2>/dev/null)"; then
	#if [ $? -eq 0 ]; then
	eval "$__mamba_setup"
else
